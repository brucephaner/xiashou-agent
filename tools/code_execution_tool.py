...
_sock = None
# The protocol has no request id and the parent handles one connection
# serially, so concurrent sandbox threads must serialize send+recv on the
# shared socket or they can receive each other's responses.
...
_seq = 0
# Guard non-atomic sequence allocation so concurrent sandbox threads do not
# clobber each other's request/response files.
